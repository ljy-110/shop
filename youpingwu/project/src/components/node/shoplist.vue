      ...
      <div class="sj-top">
       ...
       <!-- 列表数据 -->
        <el-table :data="list" border>
            ...
            <el-table-column label="名称" prop="name" :show-overflow-tooltip="true" width="50"></el-table-column>
            <el-table-column label="品牌" prop="brand_id" :show-overflow-tooltip="true" width="100"></el-table-column>
            <el-table-column label="类别" prop="class_id" :show-overflow-tooltip="true" width="100"></el-table-column>
            <el-table-column label="价格" prop="price" :show-overflow-tooltip="true" width="50"></el-table-column>
            <el-table-column label="优惠价格" prop="discount_price" :show-overflow-tooltip="true" width="50"></el-table-column>
            <el-table-column label="标题" prop="title" :show-overflow-tooltip="true"></el-table-column>
            <el-table-column label="简介" prop="text" :show-overflow-tooltip="true"></el-table-column>
            <el-table-column label="封面地址" prop="url" :show-overflow-tooltip="true"></el-table-column>
            <el-table-column label="购买人数" prop="purchase" :show-overflow-tooltip="true" width="50"></el-table-column>
            <el-table-column label="评价人数" prop="evaluate" :show-overflow-tooltip="true" width="50"></el-table-column>
            <el-table-column label="收藏人数" prop="collection" :show-overflow-tooltip="true" width="55"></el-table-column>
            <el-table-column label="添加时间" prop="time" :show-overflow-tooltip="true" width="200">
            ...
            <el-table-column label="操作" align="center" width="250">
                    <template scope="scope">
                        <el-button type="success" @click="seeInfo(scope.row.id)">查看</el-button>
                        <el-button type="primary" @click="getinfo(scope.row.id)">修改</el-button>
        ...
        <!-- 详情弹窗 -->
           <el-dialog title="查看详情" :visible.sync="seeDialog" width="30%"
            ...
            <div class="messageBox">
                <p><label>序号：</label>
                <span>{{listinfo.list_id}}</span></p>
                <p><label>名称：</label>
                <span>{{listinfo.list_name}}</span></p>
                <p><label>品牌：</label>
                <span>{{listinfo.list_brand}}</span></p>
                <p><label>类别：</label>
                <span>{{listinfo.list_class_id}}</span></p>
                <p><label>价格：</label>
                <span>{{listinfo.list_price}}</span></p>
                <p><label>优惠价格：</label>
                <span>{{listinfo.list_discountprice}}</span></p>
                <p><label>资讯标题：</label>
                <span>{{listinfo.list_title}}</span></p>
                <p><label>资讯简介：</label>
                <span>{{listinfo.list_text}}</span></p>
                <p><label>添加时间：</label>
                <span>{{listinfo.list_time}}</span></p>
                <p><label>购买人数：</label>
                <span>{{listinfo.list_purchase}}</span></p>
                <p><label>评价人数：</label>
                <span>{{listinfo.list_evaluate}}</span></p>
                <p><label>收藏人数：</label>
                <span>{{listinfo.list_collection}}</span></p>
                <label>封面详情：</label>
                <br>
           ...
           <el-dialog title="添加数据" :visible.sync="addDialog" width="30%"
            ...
            <form>
                ...
                <label>名称：</label>
                <input type="text" v-model="listinfo.list_name">
                <label>品牌：</label>
                <input type="text" v-model="listinfo.list_brand">
                <label>类别：</label>
                <input type="text" v-model="listinfo.list_class_id">
                <label>价格：</label>
                <input type="text" v-model="listinfo.list_price">
                <label>优惠价格：</label>
                <input type="text" v-model="listinfo.list_discountprice">
                <label>标题：</label>
                <input type="text" v-model="listinfo.list_title">
                <label>封面图片地址：</label>
                <input type="text" v-model="listinfo.list_img">
                <label>添加时间：</label>
                <input type="text" v-model="listinfo.list_time">
                <label>购买人数：</label>
                <input type="text" v-model="listinfo.list_purchase">
                <label>评价人数：</label>
                <input type="text" v-model="listinfo.list_evaluate">
                <label>收藏人数：</label>
                <input type="text" v-model="listinfo.list_collection">
                <label>简介：</label>
                <!-- <input type="text" v-model="listinfo.list_contents"> -->
                <textarea cols="30" rows="2" v-model="listinfo.list_text"></textarea>
            </form>
...
import { Modal } from 'ant-design-vue';
          ...
          // 弹窗关闭
            ...
            getList() {
                ...
                let list = this.data.filter((item, index) =>
                    ...
                    item.text.includes(this.searchData)  
                )
    ...
    //    修改
       putlistinfo(){
           if(this.listinfo.list_title == '' || this.listinfo.list_text == ''){
                this.flag=true
           }else{
                this.$http.put("good/goods",{
                id:this.listinfo.list_id,
                name:this.listinfo.list_name,
                brand_id:this.listinfo.list_brand,
                class_id:this.listinfo.list_class_id,
                price:this.listinfo.list_price,
                discount_price:this.listinfo.list_discountprice,
                text:this.listinfo.list_text,
            ...
            if (action === 'confirm') {
              ...
              setTimeout(() => {
                ...
                setTimeout(() => {
                  ...
                  this.$http.delete('good/goods/'+id)
                    .then(result=>{
